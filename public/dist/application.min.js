...
"use strict";angular.module("admin").run(["Menus",function(Menus){Menus.addMenuItem("topbar",{title:"Admin",state:"admin",type:"dropdown",roles:["admin"]}),Menus.addSubMenuItem("topbar","admin",{title:"Manage Users",state:"admin.users"})}]);
"use strict";angular.module("admin").config(["$stateProvider",function($stateProvider){$stateProvider.state("admin",{"abstract":!0,url:"/admin",template:"<ui-view/>"}).state("admin.users",{url:"/users",templateUrl:"modules/admin/views/list-users.client.view.html"}).state("admin.user",{url:"/users/:userId",templateUrl:"modules/admin/views/view-user.client.view.html"}).state("admin.edit-user",{url:"/users/:userId/edit",templateUrl:"modules/admin/views/edit-user.client.view.html"})}]);
"use strict";angular.module("admin").controller("AdminController",["$scope","$stateParams","$state","$filter","Authentication","Admin",function($scope,$stateParams,$state,$filter,Authentication,Admin){$scope.authentication=Authentication,$scope.remove=function(user){confirm("Are you sure you want to delete this user?")&&(user?(user.$remove(),$scope.users.splice($scope.users.indexOf(user),1)):$scope.user.$remove(function(){$state.go("admin.users")}))},$scope.update=function(){var user=$scope.user;user.$update(function(){$state.go("admin.user",{userId:user._id})},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.find=function(){Admin.query(function(data){$scope.users=data,$scope.buildPager()})},$scope.findOne=function(){$scope.user=Admin.get({userId:$stateParams.userId})},$scope.buildPager=function(){$scope.pagedItems=[],$scope.itemsPerPage=15,$scope.currentPage=1,$scope.figureOutItemsToDisplay()},$scope.figureOutItemsToDisplay=function(){$scope.filteredItems=$filter("filter")($scope.users,{$:$scope.search}),$scope.filterLength=$scope.filteredItems.length;var begin=($scope.currentPage-1)*$scope.itemsPerPage,end=begin+$scope.itemsPerPage;$scope.pagedItems=$scope.filteredItems.slice(begin,end)},$scope.pageChanged=function(){$scope.figureOutItemsToDisplay()}}]);
"use strict";angular.module("admin").factory("Admin",["$resource",function($resource){return $resource("api/users/:userId",{userId:"@_id"},{update:{method:"PUT"}})}]);
